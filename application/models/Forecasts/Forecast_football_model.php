      ...
      */
      public function to_forecast_football(){
          ...
          */
          ...
          if(empty($user_id)){
              log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' მომხმარებლის სესიის id ვერ მოიძებნა  function to_forecast_football() | '.__FILE__.' on line : '.__LINE__);
              die;
          }
          ...
          if(count($matches) > 2){
              log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' მომხმარებელის მხრიდან ან 2 ზე მეტი მატჩი მოვიდა ან 2-ჯერ იქნა გამოგზავნილი | გამყოფი function to_forecast_football() | '.__FILE__.' on line : '.__LINE__);
              die();
          }
              ...
              $match = explode('=',$matches[$i]);
                ...
                switch ($match[1]) {
                  ...
                  case 'under': $oddType = 'under';
                    break;
                    default:
                    log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' user id = ['.$this->session->admin_user_id.'] პარამეტრი : '.$match[1].' რომელიც არ შეესაბამება აქ ჩამოთვლილებს (1 X 2 over under) function to_forecast_football() | '.__FILE__.' on line : '.__LINE__);
                    die;
                      break;
                }
                  ...
                  if ($query2->num_rows() == 0){
                        log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' user id = ['.$this->session->admin_user_id.'] ასეთი მატჩი არ არსებობს  function to_forecast_football() | '.__FILE__.' on line : '.__LINE__);
                        continue;
                  }
                  ...
                  */
                  ...
                  if(floatval(str_replace(",",".",$result[0]["$oddType"])) <= 1.40){
                    ...
                    // შევიტანოთ ბაზაში მომხმარებლის პროგნოზი
                     //  $sql2 = "INSERT INTO forecasted_matches (user_id,sport_id,xml_id,match_name,odd_type,odd_value,start_date)
                     // VALUES($user_id,10,'".$result[0]['xml_id']."','".$result[0]['match_name']."',
                     // '".$oddType."','".floatval(str_replace(",",".",$result[0][$oddType]))."','".$result[0]['start_date']."')";
