...
class Forecast_hockey_model extends CI_Model {


   /**
  * ჰოკეის მატჩის პროგნოზების გაკეთება
  *
  * @param
  * @return
  */
  public function to_forecast_hockey(){
      ...
      */
      ...
      if(empty($user_id)){
          log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' მომხმარებლის სესიის id ვერ მოიძებნა  function to_forecast_hockey() | '.__FILE__.' on line : '.__LINE__);
          die;
      }
      ...
      if(!isset($_POST['matches']) || empty($_POST['matches']) ){
          ...
          log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' მომხმარებელის მხრიდან  $_POST[matches] ცარიელი მოვიდა function to_forecast_hockey() | '.__FILE__.' on line : '.__LINE__);
      ...
      if(count($matches) > 2){
          log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' მომხმარებელის მხრიდან ან 2 ზე მეტი მატჩი მოვიდა ან 2-ჯერ იქნა გამოგზავნილი | გამყოფი function to_forecast_hockey() | '.__FILE__.' on line : '.__LINE__);
          die();
      }
      ...
      for($i=0;$i< count($matches) ;$i++){
          ...
          $match = explode('=',$matches[$i]);
              ...
              if( count($match) != 2 ){
                    log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' მომხმარებელის მხრიდან ერთზე მეტი გამყოფი = იქნა გამოგზავნილი function to_forecast_hockey() | '.__FILE__.' on line : '.__LINE__);
                    die;
              }

              // მოწმდება ცარიელი ხომ არ არის მატჩის იდენთიფიკატორი და ფსონის ტიპი
              if(empty($match[0]) || empty($match[1])){
                  log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' მომხმარებელის მხრიდან ცარიელი მოვიდა ან მატჩის xml_id ან პროგნოზის ტიპი function to_forecast_hockey() | '.__FILE__.' on line : '.__LINE__);
                  die;
              }

              // მოწმდება მატჩის იდენთიფიკატორი რიცხვი არის თუ არა
              if( !filter_var($match[0], FILTER_VALIDATE_INT) ){
                  log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' user id = ['.$this->session->user_id.'] მომხმარებელმა არავალიდური მონაცემი გადასცა, მატჩის იდენტიფიკატორი რიცხვი უნდა იყოს function to_forecast_hockey() | '.__FILE__.' on line : '.__LINE__);
                  die();
              }

              // ფსონების ტიპის, მონაცემთა ბაზის სვეტების სახელებად გარდაქმნა და შემოწმება შესაბამისობაზე
              switch ($match[1]) {
                case '1':     $oddType = 'one';
                  break;
                case 'X':     $oddType = 'drow';
                ...
                case 'under': $oddType = 'under';
                  break;
                  default:
                  log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' user id = ['.$this->session->user_id.'] პარამეტრი : '.$match[1].' რომელიც არ შეესაბამება აქ ჩამოთვლილებს (1 X 2 over under) function to_forecast_hockey() | '.__FILE__.' on line : '.__LINE__);
                  die;
                    break;
              }
              ...
              // თუ ასეთი მატჩი ბაზაში ვერ მოიძებნა ლოგში ცავწეროთ შეცდომა
              if ($query2->num_rows() == 0){
                    log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' user id = ['.$this->session->user_id.'] ასეთი მატჩი არ არსებობს  function to_forecast_hockey() | '.__FILE__.' on line : '.__LINE__);
                    continue;
              }
              ...
              */
              ...
              if(floatval(str_replace(",",".",$result[0]["$oddType"])) <= 1.40){
                ...
                // შევიტანოთ ბაზაში მომხმარებლის პროგნოზი
                 ...
                 VALUES($user_id,40,'".$result[0]['xml_id']."','".$result[0]['match_name']."',
                 '".$oddType."',0.00,'".floatval(str_replace(",",".",$result[0][$oddType]))."','".$result[0]['start_date']."')";
                 $this->db->query($sql2);
    ...
    // გადავიტანე Profile_model ში
    /* მომხმარებლის მიერ გაკეთებული ფეხბურთის პროგნოზების გამოტანა */
    // public function get_my_football_forecast()
    // {
    //     $user_id = $this->session->userdata('user_id');
    //     $sql = "SELECT * FROM forecasted_matches WHERE user_id = $user_id";
    //     $query = $this->db->query($sql);
    //     return $query->result_array();
    //     //var_dum($query->result_array());
    // }
