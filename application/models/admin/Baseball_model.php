...
class Baseball_model extends CI_Model {
    ...
    public function baseball_matches_all_leagues(){


           $url = "http://ad.interwetten.com/XMLFeeder/feeder.asmx/getfeed?FEEDPARAMS=BettingOffers2|LANGUAGE=EN|KINDOFSPORTIDS=14|OFFERTYPEIDS=13,88|LEAGUEIDS=";
            // $url = "getfeed.xml";
              ...
              if(!isset($quoteType[$i]['two']) || $quoteType[$i]['two'] == '-1,00'){
                 $quoteType[$i]['two'] = '1,00';
                    ...
                    if(empty($over_under_array)){
                      $sql="
                         INSERT INTO baseball_matches (xml_id,leagues_id,match_name,one,two,start_date)
                  ...
                  for ($b=0; $b < count($over_under_array['over']['average']); $b++) {
                    ...
                    $sql="
                       INSERT INTO baseball_matches (xml_id,leagues_id,match_name,one,two,under_over_val,over,under,start_date)
                       VALUES('". $xmlstr->KINDOFSPORT->LEAGUE[$k]->EVENT[$i]['EVENTID']."',
                       '".$xmlstr->KINDOFSPORT->LEAGUE[$k]['ID']."',".$this->db->escape($xmlstr->KINDOFSPORT->LEAGUE[$k]->EVENT[$i]['NAME']).",
