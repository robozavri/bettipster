...
class Forecast_basketball_model extends CI_Model {
   ...
   * @return
   */
   public function to_basketball_football(){

     $user_id = $this->session->userdata('admin_user_id');
     if(empty($user_id)){
       log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' admin_user_id ცარიელია სესიაში function to_basketball_football() | '.__FILE__.' on line : '.__LINE__);
       die;
     }
     ...
     if(!isset($_POST['matches']) || empty($_POST['matches']) ){
         log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' მომხმარებელის მხრიდან  $_POST[matches] ცარიელი მოვიდა function to_basketball_football() | '.__FILE__.' on line : '.__LINE__);
         die;
     }
         ...
         if( !filter_var($match[0], FILTER_VALIDATE_INT) ){
           ...
           switch ($match[1]) {
             ...
             if ($query2->num_rows() == 0){
                 log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().'  ასეთი მატჩი არ არსებობს  function to_basketball_football() | '.__FILE__.' on line : '.__LINE__);
                 continue;
             }
