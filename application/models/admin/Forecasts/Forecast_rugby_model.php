          ...
          */
          ...
          for($i=0;$i< count($matches) ;$i++){
              ...
              if( !filter_var($match[0], FILTER_VALIDATE_INT) ){
                  log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' user id = ['.$this->session->admin_user_id.'] მომხმარებელმა არავალიდური მონაცემი გადასცა, მატჩის იდენტიფიკატორი რიცხვი უნდა იყოს function to_forecast_rugby() | '.__FILE__.' on line : '.__LINE__);
                  die;
              }
                ...
                switch ($match[1]) {
                  ...
                  case '2':     $oddType = 'two';
                    break;
                    default:
                    log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' user id = ['.$this->session->admin_user_id.' მომხმარებლის ტიპი '.$this->session->user_type.'] პარამეტრი : '.$match[1].' რომელიც არ შეესაბამება აქ ჩამოთვლილებს (1 X 2 over under) function to_forecast_rugby() | '.__FILE__.' on line : '.__LINE__);
                    die;
                      break;
                  ...
                  if ($query2->num_rows() == 0){
                        log_message('error', '[security] მომხმარებელი IP : '.$this->input->ip_address().' user id = ['.$this->session->admin_user_id.' მომხმარებლის ტიპი '.$this->session->user_type.'] ასეთი მატჩი არ არსებობს  function to_forecast_rugby() | '.__FILE__.' on line : '.__LINE__);
                        continue;
                  }
