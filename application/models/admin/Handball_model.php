...
class Handball_model extends CI_Model {

     public function handball_matches_all_leagues(){

       $url = "http://ad.interwetten.com/XMLFeeder/feeder.asmx/getfeed?FEEDPARAMS=BettingOffers2|LANGUAGE=EN|KINDOFSPORTIDS=1002|OFFERTYPEIDS=13,88|LEAGUEIDS=";
...
//                            $xmlstr->KINDOFSPORT->LEAGUE[$k]->EVENT[$i]->BET[$j]['TYPEID'];
                        ...
                        switch($xmlstr->KINDOFSPORT->LEAGUE[$k]->EVENT[$i]->BET[$j]['TIP']){
                         ...
                         if($xmlstr->KINDOFSPORT->LEAGUE[$k]->EVENT[$i]->BET[$j]['TYPEID'] == '88'){
                                ...
                                if(stristr($xmlstr->KINDOFSPORT->LEAGUE[$k]->EVENT[$i]->BET[$j]['PLAYER2'], 'Over')){
                                     ...
                                     $over_under_array['over']['average'][$n] = (int) $add_str_arr[1];
                                     ...
                                     $add_str_arr = explode(' ',$xmlstr->KINDOFSPORT->LEAGUE[$k]->EVENT[$i]->BET[$j]['PLAYER2']);
                                     $over_under_array['Under']['average'][$l] = (int) $add_str_arr[1];
                    ...
                    if(empty($over_under_array)){
                      $sql="
                         INSERT INTO handball_matches (xml_id,leagues_id,match_name,one,two,drow,start_date)
                         ...
                         '".$quoteType[$i]['one']."','".$quoteType[$i]['two']."','".$quoteType[$i]['drow']."','".$date->format('Y-m-d H:i:s')."'
                         )
                         ";
             ...
             */
                ...
                if(!isset($under['oddval']) || $under['oddval'] == '-1,00' || $under['oddval'] == ''){
                   $under['oddval'] = '1,00';
                    ...
                    $sql="
                       INSERT INTO handball_matches (xml_id,leagues_id,match_name,one,two,drow,under_over_val,over,under,start_date)
                       VALUES('".$xmlstr->KINDOFSPORT->LEAGUE[$k]->EVENT[$i]['EVENTID']."',
                       '".$xmlstr->KINDOFSPORT->LEAGUE[$k]['ID']."',".$this->db->escape($xmlstr->KINDOFSPORT->LEAGUE[$k]->EVENT[$i]['NAME']).",
                       '".$quoteType[$i]['one']."','".$quoteType[$i]['two']."','".$quoteType[$i]['drow']."','".($over['average'] + 0.5)."','".$over['oddval']."','".$under['oddval']."',
