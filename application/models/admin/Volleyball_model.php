...
class Volleyball_model extends CI_Model {
     ...
     public function volleyball_matches_all_leagues(){

       $url = "http://ad.interwetten.com/XMLFeeder/feeder.asmx/getfeed?FEEDPARAMS=BettingOffers2|LANGUAGE=EN|KINDOFSPORTIDS=1012|OFFERTYPEIDS=13,9905388|LEAGUEIDS=";
      //  $url = "vollayballmatches.xml";
              ...
              if(!isset($quoteType[$i]['two']) || $quoteType[$i]['two'] == '-1,00'){
                 $quoteType[$i]['two'] = '1,00';
                    ...
                    if(empty($over_under_array)){
                      $sql="
                         INSERT INTO volleyball_matches (xml_id,leagues_id,match_name,one,two,start_date)
             ...
             */
                ...
                if(!isset($under['oddval']) || $under['oddval'] == '-1,00' || $under['oddval'] == ''){
                   $under['oddval'] = '1,00';
                        ...
                        $sql="
                           INSERT INTO volleyball_matches (xml_id,leagues_id,match_name,one,two,under_over_val,over,under,start_date)
                           VALUES('". $xmlstr->KINDOFSPORT->LEAGUE[$k]->EVENT[$i]['EVENTID']."',
                           '".$xmlstr->KINDOFSPORT->LEAGUE[$k]['ID']."',".$this->db->escape($xmlstr->KINDOFSPORT->LEAGUE[$k]->EVENT[$i]['NAME']).",
                           '".$quoteType[$i]['one']."','".$quoteType[$i]['two']."','".($over['average'] + 0.5)."','".$over['oddval']."','".$under['oddval']."',
