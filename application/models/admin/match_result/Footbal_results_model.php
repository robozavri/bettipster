...
class Footbal_results_model extends CI_Model {


  /**
   * პარსავს ფეხბურთის xml-ს და ბაზაში განაახლებს შედეგებს და
   * შეამოწმებს მომხმარებლის პროგნოზი გამართლდა თუ არა
   *
   * @param
   * @return array
   */
   public function check_footbal_matches_result(){

      $url = "http://ad.interwetten.com/XMLFeeder/feeder.asmx/getfeed?FEEDPARAMS=EventResults|LANGUAGE=EN|KINDOFSPORTIDS=10";
      // $url = 'fres.xml';
  ...
  public function check_match_result($match_id,$match_result){
      ...
      /*ზოგიერთი გადადებული მატჩის შედეგი ცარიელი მოდის*/
          ...
          if( ( (int) $matchResArr[0] + (int) $matchResArr[1]) > 2.5){
                 $odd_type2 = 'over';
          }else{
                 $odd_type2 = 'under';
          }
    ...
    /*    $sql = "UPDATE forecasted_matches
                ...
                WHERE sport_id = 10 AND xml_id = ".(int) $match_id." AND status = 0
                ";
                */
