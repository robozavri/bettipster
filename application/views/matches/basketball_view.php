...
<?php endif; ?>
	...
	function sendMatch(){
			...
			if(arr[0] != undefined){
				...
				$.ajax({
				    type: "POST",
				    url: "<?php echo base_url();?>Ajax_forecast/to_forecast/3",
      ...
      if(!empty($matches)){
			foreach ($matches as $match) {
				...
				$now = time();
				 ...
				 ?>
				 <!-- data-add-val="<?php echo $match['under_over_val'];?>"
data-add-val="<?php echo $match['under_over_val'];?>" -->
					<tr data-id="<?php echo $match['xml_id'];?>" class="matchData">
						<td><?php echo $match['match_name'];?></td>
						<td class="match" data-add-val="" data-add-type="1"><?php echo check_add($match['one']);?></td>
						<td class="match" data-add-val="" data-add-type="2"><?php echo check_add($match['two']);?></td>
  ...
  $(".matchData").click(function(e){
		...
		if($(e.target).attr('data-add-type')){
		 ...
		 if($(e.target).attr('data-add-val')){
		   ...
		   switch(odd){
			 	case 'under' : showOdd = '< '+oddIndVal;
			 	break;
			 	case 'over' : showOdd = '> '+oddIndVal;
			 	break;
			 	default : showOdd = odd;
			 }
